...
https://github.com/libretro/glsl-shaders/blob/master/crt/crt-easymode-halation.glslp



re: PASSPREV4
https://forums.libretro.com/t/problem-with-crt-easymode-halation/3350
i.e. PASSPREV4 and PassPrev4Texture refers to the canvas that the linearisation shader has produced.


In the libretro shader pipeline, **PassPrev4Texture** is a built-in texture variable that provides access to the output of the fourth previous pass in the shader chain. It's one of the global texture variables that allow a shader to access the results of previous passes, giving you the ability to apply additional effects that depend on those earlier computations.

What it does:
PassPrev4Texture specifically refers to the texture produced four passes before the current shader pass.
It's useful when you're working with multi-pass shaders, where the output of one pass is fed as input to another pass. By having access to the results of earlier passes, you can perform more complex effects such as feedback loops, motion blur, bloom effects, or time-based transformations.
*/
...
////////////////////////////
// SourceSize = vec4(vec2(love_ScreenSize), 1.0/vec2(love_ScreenSize) )
////////////////////////////

// #define SourceSize vec4(TextureSize, 1.0 / TextureSize) //either TextureSize or InputSize
#define SourceSize vec4(love_ScreenSize.xy, 1.0 / love_ScreenSize.xy) //either TextureSize or InputSize
// #define outsize vec4(OutputSize, 1.0 / OutputSize)
#define outsize vec4(love_ScreenSize.xy, 1.0 / love_ScreenSize.xy)
...
#define SHARPNESS_H 0.0//0.6
#define SHARPNESS_V 0.2//1.0
#define MASK_TYPE 4.0//4.0
#define MASK_STRENGTH_MIN 0.05//0.2
#define MASK_STRENGTH_MAX 0.4
#define MASK_SIZE 1.0//1.0
#define SCANLINE_STRENGTH_MIN 0.1//0.2
#define SCANLINE_STRENGTH_MAX 0.1//0.2
#define SCANLINE_BEAM_MIN 1.0//1.0
#define SCANLINE_BEAM_MAX 1.0
#define GEOM_CURVATURE 0.02 //0.0
#define GEOM_WARP 0.05 //0.0
#define GEOM_CORNER_SIZE 0.15//0.0
#define GEOM_CORNER_SMOOTH 20//150
#define INTERLACING_TOGGLE 1.0 //1.0
#define HALATION 0.3//0.3
#define DIFFUSION 0.25//0.0
...
vec4 effect( vec4 color, Image tex, vec2 texture_coords, vec2 screen_coords)
{
    ...
    vec3 flashc = vec3(1.0, 1.0, 1.0);
    // texture_coords.y += timer * 0.001;

    // if (INTERLACING_TOGGLE > 0.5 && SourceSize.y >= 400.)
    if (INTERLACING_TOGGLE > 0.5)
    {
        // flashc = vec3(1.0, 0.0, 0.0);
        tex_size.y *= 0.5;
    ...
    tex_size.y *=2.0;
    vec2 co = texture_coords * tex_size * (1.0 / SourceSize.xy);
    ...
    vec2 mod_fac = floor(texture_coords * outsize.xy * SourceSize.xy / (SourceSize.xy * vec2(MASK_SIZE, mask_dot_height * MASK_SIZE))) * 1.0001;
